...
public class C5_1 {
	...
	static int d(Integer n) {
		int sum = 0;
		String str = n.toString();

		for (int i = 0; i < str.length(); i++) // n의 각 자리수 더함
			sum += str.charAt(i) - 48;
	...
	public static void main(String[] args) {
		HashMap<Integer, Integer> nonSelf = new HashMap<Integer, Integer>();

		for (int i = 1; i <= 10000; i++) // 해시맵 value에 key의 생성자 저장
			nonSelf.put(d(i), i);

		for (int i = 1; i <= 10000; i++)
			if (nonSelf.get(i) == null) // 생성자가 없으면 출력
